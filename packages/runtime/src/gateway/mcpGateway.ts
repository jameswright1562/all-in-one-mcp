...
import { CallToolRequestSchema, ListToolsRequestSchema, type JSONRPCMessage, isInitializeRequest } from '@modelcontextprotocol/sdk/types.js'
...
export class McpGateway {
  ...
  async handleNodeRequest(req: IncomingMessage, res: ServerResponse, parsedBody?: unknown): Promise<void> {
    const sessionIdHeader = req.headers['mcp-session-id']
    const sessionId = Array.isArray(sessionIdHeader) ? sessionIdHeader[0] : sessionIdHeader

    if (!sessionId && req.method === 'POST' && isInitializeRequest(parsedBody as JSONRPCMessage)) {
  ...
  private async createSession(): Promise<GatewaySession> {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (request) => {
      return this.runtime.callTool(request.params.name, request.params.arguments as Record<string, unknown> | undefined)
    })
