...
async function handleRequest(runtime: ManagedMcpRuntime, request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (/^\/api\/mcps\/[^/]+\/logs$/.test(pathname) && request.method === 'GET') {
    ...
    json(response, 200, {
      items: runtime.listLogs(id, normalizeLimit(url.searchParams.get('limit'))),
      generatedAt: new Date().toISOString()
    })
    return
  }

  if (/^\/api\/mcps\/[^/]+\/start$/.test(pathname) && request.method === 'POST') {
    const id = getIdFromPath(pathname)
    if (!id) {
      json(response, 400, { error: 'Missing MCP id.' })
      return
  ...
  if (/^\/api\/mcps\/[^/]+\/stop$/.test(pathname) && request.method === 'POST') {
    const id = getIdFromPath(pathname)
    if (!id) {
      json(response, 400, { error: 'Missing MCP id.' })
      return
  ...
  if (/^\/api\/mcps\/[^/]+\/restart$/.test(pathname) && request.method === 'POST') {
    const id = getIdFromPath(pathname)
    if (!id) {
      json(response, 400, { error: 'Missing MCP id.' })
      return
