...
export class ManagedMcpSupervisor {
  ...
  constructor(definition: ManagedMcpDefinition, private readonly hooks: SupervisorHooks) {
  ...
  async callTool(upstreamName: string, args: Record<string, unknown> | undefined): Promise<CallToolResult> {
    await this.start()
  ...
  private attachTransportListeners(transport: StdioClientTransport | StreamableHTTPClientTransport): void {
    ...
    transport.onclose = () => {
      if (!this.disposing && !this.stopping) {
        void this.handleFailure(new Error('Managed MCP connection closed unexpectedly.'))
      }
    }
  }
