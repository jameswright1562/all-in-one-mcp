...
<template>
  <section class="page-panel">
    ...
    <div v-else class="fleet-grid">
      <article v-for="snapshot in items" :key="snapshot.definition.id" class="fleet-card" :class="{ 'is-selected': selectedId === snapshot.definition.id }">
        ...
        <div class="fleet-card__actions">
          <button class="action-chip" type="button" :disabled="actioning" @click="$emit('action', snapshot.definition.id, 'start')">Start</button>
          <button class="action-chip" type="button" :disabled="actioning" @click="$emit('action', snapshot.definition.id, 'stop')">Stop</button>
          <button class="action-chip" type="button" :disabled="actioning" @click="$emit('action', snapshot.definition.id, 'restart')">Restart</button>
        </div>
      </article>
    </div>
  </section>
