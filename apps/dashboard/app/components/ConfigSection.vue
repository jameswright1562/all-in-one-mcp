...
import type { ConfigMode, FormState, ManagedMcpDefinition, ManagedMcpSnapshot } from '../types/dashboard'
      ...
      <section class="config-card config-card--form">
        <div class="config-card__modebar">
          ...
          <div class="segmented-control segmented-control--mode" role="tablist" aria-label="Configuration mode">
            <button class="segmented-control__option" :class="{ 'is-active': configMode === 'create' }" type="button" @click="$emit('mode-change', 'create')">
              Create New
            </button>
            <button class="segmented-control__option" :class="{ 'is-active': configMode === 'edit' }" :disabled="!selectedDefinition" type="button" @click="$emit('mode-change', 'edit')">
              Edit Selected
            </button>
          </div>
        </div>
        ...
        <form class="mcp-form" @submit.prevent="$emit('submit')">
          <div class="mcp-form__grid">
            <label class="field">
              <span>MCP ID</span>
              <input :value="createForm.id" autocomplete="off" :disabled="configMode === 'edit'" placeholder="playwright" @input="$emit('id-input', ($event.target as HTMLInputElement).value)" />
            ...
            <label class="field">
              <span>Name</span>
              <input :value="createForm.name" autocomplete="off" placeholder="Playwright MCP" @input="$emit('name-input', ($event.target as HTMLInputElement).value)" />
            ...
            <label class="field">
              ...
              <input v-model="createForm.toolPrefix" autocomplete="off" placeholder="playwright" @input="$emit('clear-error', 'toolPrefix')" />
            ...
            <label class="field">
              ...
              <input v-model.number="createForm.startupTimeoutMs" min="1" step="1000" type="number" @input="$emit('clear-error', 'startupTimeoutMs')" />
          ...
          <div class="field">
            ...
            <div class="segmented-control" role="radiogroup" aria-label="Transport selection">
              <button class="segmented-control__option" :class="{ 'is-active': createForm.transport === 'stdio' }" type="button" @click="$emit('transport-change', 'stdio')">
                stdio
              </button>
              <button class="segmented-control__option" :class="{ 'is-active': createForm.transport === 'streamable-http' }" type="button" @click="$emit('transport-change', 'streamable-http')">
                streamable-http
              </button>
            </div>
          ...
          <div v-if="createForm.transport === 'stdio'" class="mcp-form__stack">
            <label class="field">
              <span>Command</span>
              <input v-model="createForm.command" autocomplete="off" placeholder="npx" @input="$emit('clear-error', 'command')" />
          ...
          <div v-else class="mcp-form__stack">
            <label class="field">
              ...
              <input v-model="createForm.url" autocomplete="off" placeholder="http://127.0.0.1:4100/mcp" @input="$emit('clear-error', 'url')" />
          ...
          <div class="mcp-form__actions">
            <button class="action-button" type="submit" :disabled="saving || (configMode === 'edit' && !selectedDefinition)">
              {{ saving ? (configMode === 'edit' ? 'Saving...' : 'Creating...') : configMode === 'edit' ? 'Save Changes' : 'Add MCP' }}
            </button>
      ...
      <section class="config-card">
        ...
        <template v-else>
          <dl class="config-meta">
            <div>
              <dt>Status</dt>
              <dd>{{ selectedSnapshot ? selectedSnapshot.status : 'stopped' }}</dd>
            </div>
            <div>
              <dt>PID</dt>
            ...
            <div>
              <dt>Last Update</dt>
              <dd>{{ selectedSnapshot ? new Date(selectedSnapshot.updatedAt).toLocaleString() : 'N/A' }}</dd>
            </div>
            <div>
              <dt>Visible Logs</dt>
              <dd>{{ logsLength }}</dd>
          ...
          <div class="config-card__actions">
            <button class="action-button" type="button" @click="$emit('export-config')">Export Config</button>
          </div>
