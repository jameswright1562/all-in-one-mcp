...
export function useProfilesDashboard() {
  ...
  async function load(): Promise<void> {
    loading.value = true;
    try {
      const response = await $fetch<ProfileCollection>("/api/profiles");
  ...
  async function createProfile(
    ...
    saving.value = true;
    try {
      const result = await $fetch<ProfileDefinition>("/api/profiles", {
        method: "POST",
        body: profile,
      });
      await load();
  ...
  async function updateProfile(
    ...
    saving.value = true;
    try {
      const result = await $fetch<ProfileDefinition>(`/api/profiles/${id}`, {
        method: "PATCH",
        body: profile,
      });
      await load();
